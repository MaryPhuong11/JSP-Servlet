    ...
    deleteButtons.forEach(button => {
        ...
        button.addEventListener('click', function(e) {
            ...
            const productId = this.getAttribute('data-product-id') || 
                             (this.onclick ? this.onclick.toString().match(/\d+/) : null)?.[0];
            if (productId) {
    ...
    forms.forEach(form => {
        form.addEventListener('submit', function(e) {
            if (!validateForm(this)) {
                e.preventDefault();
                showAlert('Vui long kiem tra lai thong tin da nhap!');
            } else {
...
function deleteProduct(productId) {
    if (!productId) {
        showAlert('ID san pham khong hop le!');
        return;
    }
...
function showLoading(element) {
    ...
    if (element) {
        ...
        element.innerHTML = '<span class="loading"></span> Dang xu ly...';
        element.disabled = true;
        
        // Reset after 10 seconds (fallback)
        setTimeout(() => {
            if (element && element.innerHTML.includes('Dang xu ly')) {
