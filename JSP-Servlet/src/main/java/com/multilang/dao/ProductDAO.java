...
public class ProductDAO {
    ...
    public void updateProduct(Product product, Map<String, String[]> translations) {
        ...
        // Upsert translation per language; keep other languages untouched
        String upsertTranslation = """
            INSERT INTO ProductTranslation (ProductID, LanguageID, ProductName, ProductDescription)
            VALUES (?, ?, ?, ?)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try {
                ...
                // Upsert only provided translations; others remain unchanged
                try (PreparedStatement pstmt3 = conn.prepareStatement(upsertTranslation)) {
                    for (Map.Entry<String, String[]> entry : translations.entrySet()) {
                        String languageId = entry.getKey();
                        String[] values = entry.getValue();
                        String productName = values[0];
